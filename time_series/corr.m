...
% passar pra radiano de novo:
dir3=dir2./180.*pi;


[V,U]=pol2cart(dir3,vel2);
...
title('Componente Zonal (E-W) - [cm.s^{-1}]')
ylabel('[m.s^{-1}]') 

subplot(312)
plot(DATA2,V./100)
datetick('x',1);grid on
title('Componente Meridional (N-S) - [cm.s^{-1}]')
ylabel('[m.s^{-1}]') 

subplot(313)
estick(DATA2,U./100,V./100,45,'m.s^{-1}');
datetick('x',1);
title('Serie Temporal Correntografica - Sao Sebastiao')


figure
mypolar2(dir2*pi/180,vel2,'r.'); set(gcf,'color','w')
