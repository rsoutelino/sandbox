...
#elif defined PHD15 || defined PHD16 || defined PHD17 || defined PHD18 || defined PHD19 
!
!  Set T, M2 and M3 nudging coefficients in S, N edges 
!  from a 5 days time scale at the boundary point to decrease
!  linearly to 360 days 20(N,S) grids points away from the boundary.
!

      cff1=1.0_r8/(5_r8*86400.0_r8)
      cff2=1.0_r8/(360.0_r8*86400.0_r8)
      DO j=JstrR,MIN(20,JendR)
        DO i=IstrR,IendR
          wrk(i,j)=cff2+REAL(20-j,r8)*(cff1-cff2)/20.0_r8
        END DO
      END DO
      DO j=MAX(JstrR,Mm(ng)-20),JendR
        DO i=IstrR,IendR
          wrk(i,j)=cff1+REAL(Mm(ng)-j,r8)*(cff2-cff1)/20.0_r8
        END DO
      END DO
!      DO i=MAX(IstrR,Lm(ng)-20),IendR
!        DO j=JstrR,JendR
!          wrk(i,j)=cff1+REAL(Lm(ng)-i,r8)*(cff2-cff1)/20.0_r8
!        END DO
!      END DO      
