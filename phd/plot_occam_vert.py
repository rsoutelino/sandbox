...
for y in range(2003, 2004):
    ...
    etopo = sp.loadmat('/Users/rsoutelino/rsoutelino/mestrado/proc/common/etopo2_leste.mat')
    xb = etopo.pop('xb')
    yb = etopo.pop('yb')
    zb = etopo.pop('zb')
    xb,yb = m(xb,yb)
...
plt.figure(1,figsize=(14,8),facecolor='w')
...
p2.set_xlim(-37,-33); p2.set_ylim(-1000,0); 
...
con4 = p4.contourf(x,z, np.squeeze(V[:f1000,fS,:fE]*100),
    np.arange(vsc[0], vsc[1], vst), cmap=plt.cm.RdBu)
p4.set_xlim(-42,-33); p4.set_ylim(-1000,0);
p4.text(-35,-200,str(TS.round()) + ' Sv')
