...
tgmax = 0.1 # tan of the maximum angle that the transect is allowed to have to the horizontal
...
z0max = np.linspace(-350, -350, li) # keeping constant now for simplicity (Jan-2011)
v0max = np.linspace(0.3, 0.3, li) # keeping constant now for simplicity (Jan-2011)
v0 = np.zeros( [lk, li] )
...
# loading SEC-FM to fit it into the same grid
dataset = sp.loadmat('SEC_FM.mat')
zSEC    = dataset['z'];    zSEC    = zSEC[5:]
latSEC  = dataset['lat']; 
tempSEC = dataset['temp']; tempSEC = tempSEC[5:,:]
saltSEC = dataset['salt']; saltSEC = saltSEC[5:,:]
uSEC    = dataset['u'];    uSEC    = uSEC[5:,:]

latSEC  = latSEC.repeat(2, axis=1)
lonSEC = np.array([lonr.max()-3, lonr.max()+1])
lonSEC.shape  = (1, lonSEC.shape[0]); lonSEC = lonSEC.repeat(len(latSEC), axis=0)
tempSEC.shape = (tempSEC.shape[0], tempSEC.shape[1], 1); tempSEC = tempSEC.repeat(2, axis=2)
saltSEC.shape = (saltSEC.shape[0], saltSEC.shape[1], 1); saltSEC = saltSEC.repeat(2, axis=2)
uSEC.shape    = (uSEC.shape[0], uSEC.shape[1], 1);       uSEC    = uSEC.repeat(2, axis=2)
vSEC = uSEC * 0

lontot = np.hstack( (X.ravel(), lonSEC.ravel()) )
lattot = np.hstack( (Y.ravel(), latSEC.ravel()) )

# interpolating FM's to new grid
print ' '
print '======== GRIDDING NBUC-FM + SEC-FM ========'
print ' '
for k in range(0, lk-1):
    utmp = np.hstack( ( U[z[k],...].ravel(), uSEC[z[k],...].ravel() ) )
    vtmp = np.hstack( ( V[z[k],...].ravel(), vSEC[z[k],...].ravel() ) )
    ttmp = np.hstack( ( TEMP[z[k],...].ravel(), tempSEC[z[k],...].ravel() ) )
    stmp = np.hstack( ( SALT[z[k],...].ravel(), saltSEC[z[k],...].ravel() ) )
    
    print '    U:     Z Level = ' + str(-1*z[k]) + ' m'
    u[k,...] = griddata( lontot, lattot, utmp, lon, lat )
    print '    V:             = '
    v[k,...] = griddata( lontot, lattot, vtmp, lon, lat )
    print '    TEMP:          = '
    temp[k,...] = griddata( lontot, lattot, ttmp, lon, lat )
    print '    SALT:          = '
    salt[k,...] = griddata( lontot, lattot, stmp, lon, lat )
    print ' '; print ' ';
    
    del utmp, vtmp, ttmp, stmp
...
# filling up surface level with the copy of first subsurface level
u[-1,...] = u[-2,...]; v[-1,...] = v[-2,...];
...
FMtranspS = transp(lonsec, zz, np.squeeze(v[:,10]))
FMtranspN = transp(lonsec, zz, np.squeeze(v[:,-10]))
...
m.quiver(lonp, latp, u1*sc, v1*sc, scale=10)
text = "20 cm/s";
ax,ay = m( lonr.min() + 0.5, latr.max() - 3.1 ) 
m.quiver(ax,ay,0.2*sc,0,scale=10,color='k',zorder=10)
ax,ay = m( lonr.min() + 0.5, latr.max() - 3 ); 
pp1.text(ax,ay,text,color='k',fontsize=10,fontweight='bold')
...
pp1.set_title('NBUC-SEC FM - Surface Field')


pp2 = plt.subplot(1, 2, 2)
m.contourf(lonrp, latrp, zb, colors=('0.7'), alpha=0.5)
m.quiver(lonp, latp, u2*sc, v2*sc, scale=10)
text = "20 cm/s";
ax,ay = m( lonr.min() + 0.5, latr.max() - 3.1 ) 
m.quiver(ax,ay,0.2*sc,0,scale=10,color='k',zorder=10)
ax,ay = m( lonr.min() + 0.5, latr.max() - 3 ); 
pp2.text(ax,ay,text,color='k',fontsize=10,fontweight='bold')
m.drawcoastlines(zorder=5)
m.drawcountries(zorder=4)
m.fillcontinents(color=('0.8'), lake_color='aqua', zorder=3)
m.drawparallels(np.arange(latr.min(), latr.max()+2, 2),
...
m.drawmeridians(np.arange(lonr.min(), lonr.max()+2, 2),
    labels=[0, 0, 0, 1], dashes=[1,1000], zorder=7)
pp2.set_title('NBUC-SEC FM - 400 m Field')
...
sp.savemat('NBUC-SEC_FM.mat', matdict)
