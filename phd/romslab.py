...
def subset(lon, lat, var, lims):
    """
    Returns a subset z 2-D or 3-D array based on x and y limits
    USAGE: lon2, lat2, var2 = subset(lon, lat, var, lims)
    
    INPUTS: lon, lat  : 2-D arrays
            var       : ROMS netCDF4.variable object
            lims      : (xmin, xmax, ymin, ymax) [tuple]
    """
    f = np.where((lon >= lims[0]) & (lon <= lims[1]) & (lat >= lims[2]) & (lat <= lims[3]) )
    a = list(lon.shape)
    b = list(var.shape)
    
    try:
        lon2 = lon[..., f[0][0]:f[0][-1], f[1][0]:f[1][-1] ]
        lat2 = lat[..., f[0][0]:f[0][-1], f[1][0]:f[1][-1] ]
        var2 = var[..., f[0][0]:f[0][-1], f[1][0]:f[1][-1] ]
    except IndexError:
        lon2, lat2, var2 = None, None, None
        
    return lon2, lat2, var2
...
def get_hslice(nk, zlevs, var):
    """
    USAGE: varz = get_hslice(self, zlevs, nk, var)

    INPUTS:  nk    : vertical level[int, meters]
             zlevs : z levels computed from function get_depths [3D array]
             var   : the variable [3D array]
             
    OUTPUT:  varz : array-like of requested variable in requested nk for l    
    """
    km, im, jm = var.shape 
        
    varz = np.zeros((im, jm), dtype=float)# initializing array
    if nk == 0: 
        varz = var[-1,...]
    else:
#        print "\n\nInterpolating from S to Z coordinates %s\n\n" % ("."*50)
        for a in range (0, im):
            for b in range(0, jm):
                varz[a,b] = np.interp(-nk, zlevs[:, a, b], var[:, a, b], 
                                      left=np.nan )
    return varz
