...
PATHNAME = ['/Users/rsoutelino/rsoutelino/phd/data/proab1/ctd'];
DATE = 'Set, 2007';
...
nest = pb(:,1); lat = pb(:,3); lon = pb(:,2); 
prof = pb(:,4); clear posicoes pb; % when available
...
text(lon(find(nest==8323))+0.2,lat(find(nest==8323)),'1'); 
text(lon(find(nest==8325))+0.2,lat(find(nest==8325)),'2');
text(lon(find(nest==8346))+0.2,lat(find(nest==8346)),'3');
text(lon(find(nest==8349))+0.2,lat(find(nest==8349)),'4');  
text(lon(find(nest==8363))+0.5,lat(find(nest==8363)),'5');  
text(lon(find(nest==8364))+0.2,lat(find(nest==8364)),'6'); 

RR = menu('Pick Transect','# 1','# 2','# 3','# 4','#5','#6');
set(p1,'visible','off')
...
for k=1:lx;
  XX = num2str(xx(k));
  eval(['load ',PATHNAME,'/proab1_wb',XX,'.asc']);
  dados = eval(['proab1_wb',XX]);
  eval(['clear proab1_wb',XX]);
