...
PATHNAME = ['/Users/rsoutelino/rsoutelino/phd/data/abrolhos2/ctd'];
DATE = 'Mar, 2005';
...
text(lon(find(nest==7638))+0.2,lat(find(nest==7638)),'1'); 
text(lon(find(nest==7639))+0.2,lat(find(nest==7639)),'2');
text(lon(find(nest==7678))+0.2,lat(find(nest==7678)),'3');
text(lon(find(nest==7689))+0.2,lat(find(nest==7689)),'4');  
text(lon(find(nest==7705))-0.5,lat(find(nest==7705)),'5');  
text(lon(find(nest==7715))+0.2,lat(find(nest==7715)),'6'); 
...
xx = [7653:7678];
elseif RR==4;  % parallelal to the coast    (S --> N)
xx = [7679:7689];
...
for k=1:lx;
  XX = num2str(xx(k));
  eval(['load ',PATHNAME,'/abrolhos2_ctd_wb',XX,'.asc']);
  dados = eval(['abrolhos2_ctd_wb',XX]);
  eval(['clear abrolhos2_ctd_wb',XX]);
