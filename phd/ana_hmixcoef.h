...
# elif defined SCB || defined PHD15 || defined PHD16 || defined PHD17 || defined PHD18 || defined PHD19 
!
!  Southern California Bight sponge areas:

#  if defined UV_VIS2
!
!  Increase harmonic vicosity linearly (up to a factor of 4)
!  from the interior to the open boundary with a sponge area of 10 grid
...
!
      ...
      DO j=JstrR,MIN(10,JendR)
        cff=visc2(ng)+REAL(10-j,r8)*(fac*visc2(ng)-visc2(ng))/10.0_r8
        DO i=IstrR,IendR
          visc2_r(i,j)=cff
          visc2_p(i,j)=cff
        END DO
      ...
      DO j=MAX(JstrR,Mm(ng)+1-10),JendR
        cff=fac*visc2(ng)+                                              &
     &      REAL(Mm(ng)+1-j,r8)*(visc2(ng)-fac*visc2(ng))/10.0_r8
        DO i=IstrR,IendR
          visc2_r(i,j)=cff
          visc2_p(i,j)=cff
        END DO
...
#  if defined TS_DIF2
!
!  Increase harmonic diffusion linearly (up to a factor of 4)
!  from the interior to the open boundary with a sponge area of 10 grid
!  points. Notice that the sponge area is only applied at the southern,
...
!
      ...
      DO j=JstrR,MIN(10,JendR)
        cff1=tnu2(itemp,ng)+                                            &
     &       REAL(10-j,r8)*(fac*tnu2(itemp,ng)-tnu2(itemp,ng))/10.0_r8
        cff2=tnu2(isalt,ng)+                                            &
     &       REAL(10-j,r8)*(fac*tnu2(isalt,ng)-tnu2(isalt,ng))/10.0_r8
        DO i=IstrR,IendR
          diff2(i,j,itemp)=cff1
          diff2(i,j,isalt)=cff2
        END DO
     ...
     &                            fac*tnu2(isalt,ng))/10.0_r8
      ...
      DO i=IstrR,MIN(10,IendR)
        DO j=MAX(JstrR,i),MIN(Mm(ng)+1-i,JendR)
          cff1=tnu2(itemp,ng)+                                          &
     &         REAL(10-i,r8)*(fac*tnu2(itemp,ng)-tnu2(itemp,ng))/10.0_r8
          cff2=tnu2(isalt,ng)+                                          &
     &         REAL(10-i,r8)*(fac*tnu2(isalt,ng)-tnu2(isalt,ng))/10.0_r8
          diff2(i,j,itemp)=cff1
          diff2(i,j,isalt)=cff2
        END DO
      END DO
