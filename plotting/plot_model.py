...
def subplotfig(sbplt, d, s, name, area, panel, title):
    ...
    if area == 1:
        m = m1
        etopo1.mlon, etopo1.mlat = m( etopo1.lon, etopo1.lat )
    else:
        m = m2
        etopo1.mlon, etopo1.mlat = m( etopo1.lon, etopo1.lat )
...
pathname2 = "/home/rsoutelino/rsoutelino/prod/csr_wilton/"
...
# AVISO AREA 1 =======================================================
# Hardcoded for filenames due to the nature of the 
# MAT files provided by Wilton
aviso1     = Dataset()
lat        = sp.loadmat('%saviso_files/ay1.mat' % (pathname2) ); lat = lat.pop('ay1')
lon        = sp.loadmat('%saviso_files/ax1.mat' % (pathname2) ); lon = lon.pop('ax1')
lon, lat   = np.meshgrid(lon, lat)
aviso1.lat = lat
aviso1.lon = lon
aviso1.mlon, aviso1.mlat = m1(aviso1.lon, aviso1.lat)
ssh = sp.loadmat('%saviso_files/assh1.mat' % (pathname2) ); aviso1.ssh = ssh.pop('assh1')
u   = sp.loadmat('%saviso_files/au1.mat' % (pathname2) );   aviso1.u = u.pop('au1')
v   = sp.loadmat('%saviso_files/av1.mat' % (pathname2) );   aviso1.v = v.pop('av1')
# masking 
aviso1.u   = apply_mask(aviso1.u)
aviso1.v   = apply_mask(aviso1.v)
aviso1.ssh = apply_mask(aviso1.ssh)
# scaling
aviso1.u /= 100; aviso1.v /= 100

# AVISO AREA 2 =======================================================
# Hardcoded for pathnames and filenames due to the nature of the 
# MAT files provided
aviso2     = Dataset()
lat        = sp.loadmat('%saviso_files/ay2.mat' % (pathname2) ); lat = lat.pop('ay2')
lon        = sp.loadmat('%saviso_files/ax2.mat' % (pathname2) ); lon = lon.pop('ax2')
lon, lat   = np.meshgrid(lon, lat)
aviso2.lat = lat
aviso2.lon = lon
aviso2.mlon, aviso2.mlat = m2(aviso2.lon, aviso2.lat)
ssh = sp.loadmat( '%saviso_files/assh2.mat' % (pathname2) ); aviso2.ssh = ssh.pop('assh2')
u   = sp.loadmat( '%saviso_files/au2.mat' % (pathname2) );   aviso2.u = u.pop('au2')
v   = sp.loadmat( '%saviso_files/av2.mat' % (pathname2) );   aviso2.v = v.pop('av2')
# masking 
aviso2.u   = apply_mask(aviso2.u)
aviso2.v   = apply_mask(aviso2.v)
aviso2.ssh = apply_mask(aviso2.ssh)
# scaling
aviso2.u /= 100; aviso2.v /= 100;

# HYCOM AREA 1 ========================================================
# Hardcoded for pathnames and filenames due to the nature of the 
# MAT files provided
hycom1     = Dataset()
lat        = sp.loadmat('%shycom_files/y1.mat' % (pathname2) ); lat = lat.pop('y1')
lon        = sp.loadmat('%shycom_files/x1.mat' % (pathname2) ); lon = lon.pop('x1')
lon, lat = np.meshgrid(lon, lat)
hycom1.lat = lat
hycom1.lon = lon
hycom1.mlon, hycom1.mlat = m1(hycom1.lon, hycom1.lat)
u    = sp.loadmat('%shycom_files/u1.mat' % (pathname2) );   hycom1.u = u.pop('u1')
v    = sp.loadmat('%shycom_files/v1.mat' % (pathname2) );   hycom1.v = v.pop('v1')
ssh  = sp.loadmat('%shycom_files/ssh1.mat' % (pathname2) ); hycom1.ssh = ssh.pop('ssh1')
# masking 
hycom1.u   = apply_mask(hycom1.u)
hycom1.v   = apply_mask(hycom1.v)
hycom1.ssh = apply_mask(hycom1.ssh)
# scaling
hycom1.u /= 100; hycom1.v /= 100
hycom1.ssh *= 100
hycom1.ssh += 4
...
# MAT files provided
hycom2     = Dataset()
lat        = sp.loadmat('%shycom_files/y2.mat' % (pathname2) ); lat = lat.pop('y2')
lon        = sp.loadmat('%shycom_files/x2.mat' % (pathname2) ); lon = lon.pop('x2')
lon, lat = np.meshgrid(lon, lat)
hycom2.lat = lat
hycom2.lon = lon
hycom2.mlon, hycom2.mlat = m2(hycom2.lon, hycom2.lat)
u    = sp.loadmat('%shycom_files/u2.mat' % (pathname2) );   hycom2.u = u.pop('u2')
v    = sp.loadmat('%shycom_files/v2.mat' % (pathname2) );   hycom2.v = v.pop('v2')
ssh  = sp.loadmat('%shycom_files/ssh2.mat' % (pathname2) ); hycom2.ssh= ssh.pop('ssh2')
# masking 
hycom2.u   = apply_mask(hycom2.u)
hycom2.v   = apply_mask(hycom2.v)
hycom2.ssh = apply_mask(hycom2.ssh)
# scaling
hycom2.u /= 100; hycom2.v /= 100
hycom2.ssh *= 100
