...
p = 50; %input('Escolha a prof de interesse (20 a 460 de 10 em 10): ')
n = near(zc,p,1);
u = u(n,:); v = v(n,:);
mod = sqrt(u.^2+v.^2);
...
% removendo radial espuria 
%  lon = lon([1:104 150:end]);
%  lat = lat([1:104 150:end]);
%  u = u([1:104 150:end]);
%  v = v([1:104 150:end]);
...
title('Observed \Psi - OEII [m^2 s^{-1}]','fontsize',10,'fontweight','bold');
%  cc = colorbar;
%  pos = get(cc,'Position');
...
set(gcf,...
        ...
        'Clipping','on');
hold on
m_contourf(xgi,ygi,psiob,50);hold on; shading flat
caxis([-30000 30000])
%  m_contour(xgi,ygi,psiob,lpsi2,'k'); 
m_quiver(xgi(1:1:end),ygi(1:1:end),uo(1:1:end)*s,vo(1:1:end)*s,0,'k');
