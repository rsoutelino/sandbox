...
plt.plot(lon[0,:], lat[0,:], 'b.',linewidth=2)
plt.plot(lon[-1,:], lat[-1,:], 'b.',linewidth=2)
plt.plot(lon[:,0], lat[:,0], 'b.',linewidth=2)
plt.plot(lon[:,-1], lat[:,-1:], 'b.',linewidth=2)
plt.plot(xc, yc, 'k', linewidth=2)
plt.axis('equal')
plt.axis([lon.min()-1, lon.max()+1, lat.min()-1, lat.max()+1])
llc, ulc, urc, lrc = plt.ginput(4, timeout=0)

m1, h = np.polyfit(  [llc[0], lrc[0]] , [llc[1], lrc[1]] ,1 )
...
while Latr.max() <= ulc[1]:
    x = np.linspace( x0+dx, xend+dx, len(x1) )
    y = np.linspace( y0+dy, yend+dy, len(y1) )
    
    Lonr = np.vstack( (Lonr, x) )
    Latr = np.vstack( (Latr, y) )
...
ncfile = nc.Dataset(run.rundir + run.run_name + filenamestr, mode='w',
