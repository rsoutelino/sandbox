...
for k=1:lx;
  ...
  eval(['load filtrados/lesteII_ctd',XX,'.dat']);
  ...
  T(1:length(f),k)=t(f);
  S(1:length(f),k)=s(f);
  zb(k)=p(length(p)); % a profundidade maxima é cosiderada a ultima perfilada
%    pmax(k)=profest(find(nest==xx(k))); % prof. max. é a profundidade local

  lats(k)=-lat(find(nest==xx(k)));
  lons(k)=-lon(find(nest==xx(k)));
...
titvg = ['Velocidade Geostrofica OEII - ',RADIAL,' (m s^{-1})'];
lV = -2:0.05:2;
...
%  eval(['save filtrados/lesteII_agp_',radial,'.mat agp xi z xbi zbi'])
%  eval(['save filtrados/lesteII_vg_',radial,'.mat vg xiv z xbi zbi'])
%  eval(['save filtrados/lesteII_T_',radial,'.mat Ti xi z xbi zbi'])
%  eval(['save filtrados/lesteII_S_',radial,'.mat Si xi z xbi zbi'])
