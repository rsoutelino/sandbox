...
%  	m_contour(xgi,ygi,psi,lpsi2,'w')
	...
	[c,h]=m_contourf(xb,yb,zb,[-pp -pp],'k');
        set(h,'facecolor',[.7 .7 .7]);
        [c,h]=m_contour(xb,yb,zb,[-pp -pp],'k');
        set(h,'color',[.7 .7 .7]);
	...
	set(cc,'Position',[pos(1) pos(2)*1.3 pos(3)/2.5 pos(4)/1.1])
        ...
        eval(['print -depsc figuras/psi_ref_adcp_OEII_',num2str(pp),'.eps'])
        eval(['!epstopdf figuras/psi_ref_adcp_OEII_',num2str(pp),'.eps'])

        clear Uctd Vctd ugo vgo U V psi c h xcont ycont xf yf xg2 yg2 inc lpsi int text cc pos
