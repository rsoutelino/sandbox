...
caxis([lv1(1) lv1(end)]); cc = colorbar;
  colormap(jet2);
  pos = get(cc,'Position');
  set(cc,'Position',[pos(1) pos(2)*1.8 pos(3)/2 pos(4)/1.28])
axis([min(min(xg)) max(max(xg)) -pplot 0])
xlabel('Distancia ao longo da Radial [km]','fontsize',12)
ylabel('Profundidade [m]','fontsize',12)
tit = ['Radial ',num2str(RR),' - Vel. Geo Ref. por ADCP [m s^{-1}]'];
title(tit,'fontweight','bold','fontsize',12)
pbaspect([1 0.6 1])

eval(['print -depsc /home/rafaelgs/mestrado/apresentacoes/figuras/sec_adcp_objmap_rad',num2str(RR),'.eps'])
eval(['!epstopdf /home/rafaelgs/mestrado/apresentacoes/figuras/sec_adcp_objmap_rad',num2str(RR),'.eps'])
