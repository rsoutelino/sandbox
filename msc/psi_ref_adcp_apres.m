...
%  	m_contour(xgi,ygi,psi,lpsi2,'w')
	...
	m_quiver(-40,-11,0.5*fc,0,0,'w')
	text = ['50 cm s^{-1}'];
	m_text(-40,-11.3,text,'color','w','fontsize',16,'fontweight','bold')
	text = [num2str(pp),' m'];
	m_text(-34.8,-20.1,text,'color','r','fontweight','bold','fontsize',16)
	title('\Psi Geostrofico Referenciado por ADCP - OEII [m^2 s^{-1}]','fontweight','bold','fontsize',16);
	text = ['Contornos: ',int,' m^2s^{-1}'];
	m_text(-40.5,-11.8,text,'color','w','fontsize',16,'fontweight','bold')
	m_grid('box','fancy','yaxislocation','left','xaxislocation','bottom','fontsize',16);
%  	cc = colorbar;
%  	pos = get(cc,'Position');
%  	set(cc,'Position',[pos(1) pos(2)*1.3 pos(3)/2.5 pos(4)/1.1])
%          drawnow
...
%          eval(['save mat/uv_psi_ref_OEII_',num2str(pp),'m.mat psi U V'])
        ...
        eval(['print -depsc /home/rafaelgs/mestrado/apresentacoes/figuras/psi_ref_adcp_OEII_',num2str(pp),'m.eps'])
        eval(['!epstopdf /home/rafaelgs/mestrado/apresentacoes/figuras/psi_ref_adcp_OEII_',num2str(pp),'m.eps'])
