...
% carregando a grade lesteII para plotar aqui
load ../../common/seagrid_leste2.dat
xgoe2 = seagrid_leste2(:,9);
ygoe2 = seagrid_leste2(:,10);
l1 = max(seagrid_leste2(:,1));
l2 = max(seagrid_leste2(:,2));
xgoe2 = reshape(xgoe2,l2,l1);
ygoe2 = reshape(ygoe2,l2,l1);
...
set(h,'color',[.7 .7 .7])
m_quiver(xgi,ygi,uo*fc,vo*fc,0,'w')
fill(xf,yf,[0.8 0.8 0.8]);
m_plot(xgoe2(1,:),ygoe2(1,:),'r','linewidth',2)
m_plot(xgoe2(end,:),ygoe2(end,:),'r','linewidth',2)
m_plot(xgoe2(:,end),ygoe2(:,end),'r','linewidth',2)
m_usercoast('../../common/costa_larga.mat','patch',[0 0 0])
vmax = round(max(max(sqrt((uo*100).^2+(vo*100).^2))));
m_quiver(-48,-5,0.2*fc,0,0,'w')
text = ['20 cm s^{-1}'];
m_text(-48,-6,text,'color','w','fontsize',10,'fontweight','bold')
text = [num2str(pi),' m'];
m_text(-48,-10,text,'color','y','fontweight','bold')
text = ['Contornos: ',int,' m^2s^{-1}'];
m_text(-49,-13.8,text,'color','w','fontsize',8,'fontweight','bold')
m_grid('box','fancy','yaxislocation','left','xaxislocation','bottom','fontsize',8);
...
set(cc,'Position',[pos(1) pos(2)*1.6 pos(3)/2.5 pos(4)/1.2])
   print(4,'-depsc',['../figuras_larga_escala/psi_woa_',time,'_',num2str(pi),'m']);
   eval(['!epstopdf ../figuras_larga_escala/psi_woa_',time,'_',num2str(pi),'m.eps'])
drawnow

%  eval(['save ../mat/psi_levitus_largaesc_',num2str(pi),'m.mat psigo'])
