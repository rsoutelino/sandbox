...
%%% grade curvilinear x erro de interpolacao
...
%  m_plot(xgi,ygi,'w')
%  for k = 1:li
%     m_plot(xgi(k,:),ygi(k,:),'w')
%  end
%  
%  fill(xf,yf,[0.8 0.8 0.8]);
%  %  plot(xf,yf,'color',[.8 .8 .8])
%  m_plot(lonctd,latctd,'ko','markersize',6,'markerfacecolor',[1 0.689 0.127],'markeredgecolor','k')
%  m_usercoast('costa_leste.mat','patch',[0 0 0])
%  %  vmax = round(max(max(sqrt((uo*100).^2+(vo*100).^2))));
%  %  m_quiver(-40,-11,0.2*fc,0,0,'w')
%  %  text = ['20 cm s^{-1}'];
...
%  pos = get(cc,'Position');
%  set(cc,'Position',[pos(1) pos(2)*1.3 pos(3)/2.5 pos(4)/1.1])
%     print(5,'-depsc',['../figuras/er_OEII_',nn,'m']);
%     eval(['!epstopdf ../figuras/er_OEII_',nn,'m.eps'])
%  drawnow
%  %     print(4,'-depsc',['../figuras/psi_OEII_',nn,'m_3gr']);
%  %     eval(['!epstopdf ../figuras/psi_OEII_',nn,'m_3gr.eps'])
